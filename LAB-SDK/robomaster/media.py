...
class LiveView:
    ...
    def start_video_stream(self, display=True, addr=None, ip_proto="udp"):
        ...
        self._video_decoder_thread = threading.Thread(
            target=self._video_decoder_task, daemon=True
        )
    ...
    def start_audio_stream(self, addr=None, ip_proto="udp"):
        ...
        self._audio_decoder_thread = threading.Thread(
            target=self._audio_decoder_task, daemon=True
        )
    ...
    def read_video_frame(self, timeout=3, strategy="pipeline"):
        if strategy == "newest":
            newest = self._video_frame_queue.get(timeout=timeout)
            while True:
                try:
                    newest = self._video_frame_queue.get_nowait()
                except queue.Empty:
                    return newest
