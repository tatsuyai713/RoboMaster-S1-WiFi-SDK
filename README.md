...
両方が`robomaster`互換パッケージを提供しますが、SOLO SDKとLAB-SDKの間にpackage/import依存はありません。
LAB-SDKはAppID、QR、UDP/DUSS、camera/audio用の基礎transportを自身のpackage内に持ちます。
同名facadeの衝突を避けるため、用途ごとにvirtualenvを分け、選択したSDKだけをinstallしてください。
...
- H.264表示、Opus/48 kHz PCM機体音声受信、PC microphone送信
...
s1_robot.initialize(conn_type="sta", enter_solo=True)
try:
...
SOLO環境は`SDK/`だけ、LAB環境は`LAB-SDK/`だけをinstallします。相互依存はなく、
同名facadeの衝突を避けるためbackendごとにvirtualenvを分離してください。
...
| Camera / audio | 公式LiveView | App互換raw H.264、Opus、48 kHz PCM | 独立基礎transportのraw H.264、Opus、48 kHz PCM |
...
s1_robot.initialize(conn_type="sta")  # Lab enter/upload/startも自動実行
try:
    s1_robot.chassis.move(x=0.5, xy_speed=0.3).wait_for_completed()
finally:
    s1_robot.chassis.stop()
    s1_robot.close()
```
...
This is an experimental RoboMaster S1 Python project. It recreates an official-SDK-like API over the S1 Windows App-compatible Wi-Fi path. `SDK/` sends reconstructed App/DUSS traffic directly from the host. The standalone `LAB-SDK/` has its own base transport, enters Lab mode, uploads and starts a robot-side Python bridge, sends allowlisted commands over UDP `40923`, and returns measured chassis/gimbal telemetry over UDP `40924`. Neither SDK package imports or depends on the other. Both expose the captured S1 Opus/48 kHz PCM audio path.
